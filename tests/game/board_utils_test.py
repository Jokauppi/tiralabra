import pytest
import numpy as np
from game.board_utils import Utils
...
class TestBoardUtilsLinePush:
    ...
    def test_line_push_no_change(self):
        ...
        (pushed, _, _) = self.utils.push_line(initial)
    ...
    def test_line_push_not_modified(self):
        ...
        (_, modified, _) = self.utils.push_line(initial)
        assert not modified
    ...
    def test_line_push_score_no_moves(self):
        ...
        (_, _, score) = self.utils.push_line(initial)
        assert score == 0
...
class TestBoardUtilsLinePull:
    ...
    def test_line_pull_no_change(self):
        ...
        (pulled, _, _) = self.utils.pull_line(initial)
    ...
    def test_line_pull_not_modified(self):
        ...
        (_, modified, _) = self.utils.pull_line(initial)
        assert not modified
    ...
    def test_line_pull_score_no_moves(self):
        initial = np.array([2, 4, 2, 4])
        (_, _, score) = self.utils.pull_line(initial)
        assert score == 0
...
class TestIsLineMovable:
    ...
    def test_line_movability_one_tile(self):
        ...
        assert self.utils.is_line_movable(line)
    ...
    def test_line_movability_line_full_not_combinable(self):
        ...
        assert not self.utils.is_line_movable(line)
...
class TestBoardToString:
    ...
    def test_to_string_with_score(self):
        board = Board(1, initial=np.array([[2, 0, 0, 0], [0, 64, 0, 0], [
                      0, 256, 0, 0], [0, 0, 2048, 0]]), score=40000)
        assert True
